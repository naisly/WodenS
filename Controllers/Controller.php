...
class Controller
{
    ...
    public function actionGetCategories( $category ) {
        $this->actionGetCategoriesInc( $category );
    }

    /*
     * Search method for products page
     */

    private function actionGetCategoriesInc( $category ) {
    ...
    public function actionGetDistinctCategories() {
        $this->actionGetDistinctCategoriesInc();
    }

    /*
     * Method for displaying different categories
     */
    private function actionGetDistinctCategoriesInc() {
        include_once('/../Storage.php');
        $db = Storage::getInstance();
        $mysqli = $db->getConnection();
    ...
    public function actionSetAveragePrice( $category ) {
        $this->actionSetAverage( $category );
    }

    /*
     * Method for setting average price
     * between products in categories
     */

    private function actionSetAverage( $category ) {
        $i = 0;
        include_once('/../Storage.php');
        $db = Storage::getInstance();
        $mysqli = $db->getConnection();
    ...
    public function actionGetItemNames( $category ) {
        $this->actionGetItemNamesInc( $category );
    }

    /*
     * Getting distinct Item names
     * for Selling page
     */
    private function actionGetItemNamesInc( $category ) {
        $db = Storage::getInstance();
        $mysqli = $db->getConnection();
        if ( $category === 'All') {
    ...
    /*
     * Action for summing products in cart page
     */
